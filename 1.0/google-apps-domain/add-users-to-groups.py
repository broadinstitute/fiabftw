...
def add_svc_acct_to_proxy_group(service, env, domain, google_proj):
    print("adding user firecloud-{0}@{1}.iam.gserviceaccount.com to proxy_servacct@{2}".format(env, google_proj, domain))
    add_user_to_group(service, "firecloud-{0}@{1}.iam.gserviceaccount.com".format(env, google_proj),
                      "proxy_servacct@{0}".format(domain))
...
if __name__ == '__main__':
    ...
    add_svc_acct_to_proxy_group(directory, env, domain, google_proj)
