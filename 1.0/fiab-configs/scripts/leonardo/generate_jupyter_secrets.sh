...
# Used for spinning up new Leonardo environments.
#
...
vault write secret/dsde/firecloud/${ENV}/leonardo/jupyter jupyter-server.crt=@jupyter-server.crt jupyter-server.key=@jupyter-server.key leo-client.p12.b64=@leo-client.p12.b64 rootCA.key=@rootCA.key rootCA.pem=@rootCA.pem
